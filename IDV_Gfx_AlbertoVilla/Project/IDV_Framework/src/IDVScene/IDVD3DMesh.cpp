...
void D3DXMesh::Create() {
	...
	bdesc = { 0 };
	bdesc.ByteWidth = sizeof(Parser::Coordinates) * Mesh.iVertexCount;
	bdesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	D3D11_SUBRESOURCE_DATA subData = { &Mesh.coords[0], 0, 0 };
	...
	bdesc = { 0 };
	bdesc.ByteWidth = 3 * Mesh.iIndexCount * sizeof(USHORT);
	bdesc.BindFlags = D3D11_BIND_INDEX_BUFFER;

	subData = { &Mesh.index[0], 0, 0 };
...
void D3DXMesh::Draw(float *t, float *vp) {
	...
	UINT offset = 0;
	UINT stride = sizeof(Parser::Coordinates);
	...
	D3D11DeviceContext->DrawIndexed(3 * Mesh.iIndexCount, 0, 0);
}
